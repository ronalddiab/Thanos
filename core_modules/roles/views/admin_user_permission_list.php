                        ...
                        <tbody>
                            ...
                            foreach ($matrix_permissions as $key=>$value) {
                                if($value['parent_id'] == 0){
                                    ...
                                    $matrix_permissions_tree[$value['parent_id']]['children'][$value['id']] = $value;
                                }
                            }
						$permissionLableArray = array();
                            foreach ($matrix_permissions_tree as $value) {
                                ...
                                <tr>
									...
									'class' => 'check_box icheck selectparent' . $value['id'],
                                ...
                                if(!empty($value['children'])){
									...
									if ($value['permission_lable'] == 'Admin.Permissions.Index' || $value['permission_lable'] == 'Admin.Roles.Index') {
										array_push($permissionLableArray, $value1['permission_lable']);
									}
									if ($value1['permission_lable'] != 'Admin.Permissions.Delete' && $value1['permission_lable'] != 'Admin.Roles.Action.Delete') {
										if (in_array($value1['permission_lable'], $permissionLableArray)) {
							...
							?>
									<tr <?php echo $style; ?>>
                                            ...
                                            <?php
										if ($value1['permission_title'] == 'Role Delete' || $value1['permission_title'] == 'Permission Delete') {
											$childclass = "";
										} else {
											$childclass = 'selectchild' . $value1['parent_id'];
										}
											...
											'class' => 'check_box icheck ' . $childclass,
...
<script type="text/javascript">
	...
	$('.selectparent11').on('ifChecked', function() {
		if ($('.selectparent11').filter(':checked').length == $('.selectparent11').length) {
			$('.selectchild11').iCheck('check');
		}
	});

	$('.selectparent11').on('ifUnchecked', function(event) {
		$('.selectchild11').iCheck('uncheck');
	});

	$('.selectparent5').on('ifChecked', function() {
		if ($('.selectparent5').filter(':checked').length == $('.selectparent5').length) {
			$('.selectchild5').iCheck('check');
		}
	});

	$('.selectparent5').on('ifUnchecked', function(event) {
		$('.selectchild5').iCheck('uncheck');
	});

    $('.selectparent414').on('ifChecked', function() {
		if ($('.selectparent414').filter(':checked').length == $('.selectparent414').length) {
			$('.selectchild414').iCheck('check');
			$('.selectchild415').iCheck('check');
			$('.selectparent415').iCheck('check');
		}
	});

	$('.selectparent414').on('ifUnchecked', function(event) {
		$('.selectchild414').iCheck('uncheck');
		$('.selectchild415').iCheck('uncheck');
        $('.selectparent415').iCheck('uncheck');
	});
