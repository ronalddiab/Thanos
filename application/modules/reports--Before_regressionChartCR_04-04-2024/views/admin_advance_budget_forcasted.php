...
<script>
	...
	<?php if (!empty($reportdata)) {  ?>
		...
		function drawHighchart() {
			<?php if ($data['time_type'] == 'advance_select_choose_year') { ?>
				var utilityBudgetReportArray = [];
				utilityBudgetReportArray.push(['<?php echo lang("month"); ?>',
				...
				foreach ($resultkeys as $year => $value) {
					foreach ($value as $key1 => $month) {
						...
						// $forecastdata = round($forecastdata,2);  

						$total_months_pre++;
						$previousdata_tooltip_array = array();
						$currentdata_tooltip_array = array();
						$budget_tooltip_array = array();
						$previousdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($filters['start_year'] - 1) . '</strong><br>' . 'Year - ' . ($filters['start_year'] - 1) . ': ' . number_format($previousdata, 2, '.', ',') . '</br>';
						$currentdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . 'Year - ' . $filters['start_year'] . ':' . number_format($currentdata, 2, '.', ',') . '</br>';
						$budgetdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . lang('budget') . ': ' . $budgetdata . '</br>';

						$previousdata_tooltip_array[] = $previousdata_tooltip;
						$currentdata_tooltip_array[] = $currentdata_tooltip;
						$budget_tooltip_array[] = $budgetdata_tooltip;

						// For average up to data available
						if ($filters['CURRENT_YEAR_MAX_MONTH_ID'] >= $month) {
							$total_sum_previous += $previousdata;
							$total_sum_current += $currentdata;
							$total_sum_budget += $budgetdata;
							$total_sum_forecast += $forecastdata;
				...
				?>
						...
						for ($i = 1; $i <= count($fullmontharray); $i++) {
							...
							$budgetArray[] = $montharray[$i] . " " . ($filters['start_year']);
						}
						$previousArray[] = "Average";
						$currentArray[] = "Average";
						?>
						utilityBudgetReportArray.push(["<?php echo $monthdata; ?>", <?php echo $previousdata; ?>, <?php echo $currentdata; ?>, <?php echo $budgetdata; ?>]);
				<?php
					}
				}

				if ($total_months > 0) {
					$previousAvgData = ($total_sum_previous / $total_months);
					$currentAvgData = ($total_sum_current / $total_months);
					$budgetAvgData = ($total_sum_budget / $total_months);
					$forecastAvgData = ($total_sum_forecast / $total_months);
				...
				$forecastAvgData = round($forecastAvgData, 2);
				$cddAvgData = round($cddAvgData, 2);
				$hddAvgData = round($hddAvgData, 2);
				$occupancyAvgData = round($occupancyAvgData, 2);

				$previousAvgData_tooltip = '<strong>' . lang('average') . '</strong><br>' . 'Year - ' . ($filters['start_year'] - 1) . ': ' . number_format($previousAvgData, 2, '.', ',') . '</br>';
				$currentAvgData_tooltip  = '<strong>' . lang('average') . '</strong><br>' . 'Year - ' . $filters['start_year'] . ': ' . number_format($currentAvgData, 2, '.', ',') . '</br>';

				$previousdata_tooltip_array[0][] = $previousAvgData_tooltip;
				$currentdata_tooltip_array[0][] = $currentAvgData_tooltip;

				?>
				utilityBudgetReportArray.push(["<?php echo lang('average'); ?>", <?php echo $previousAvgData; ?>, <?php echo $currentAvgData; ?>, <?php echo $budgetAvgData; ?>]);
				...
				var budgettooltipArray = <?php echo json_encode($budgetdata_tooltip_array[0]); ?>;
				var currentmonthArray = <?php echo json_encode($currentArray); ?>;
				var utilityChartBudgetData = [];
				var xAxisutilityBudgetChart = [];
				var utilityChartBudgetTitle = utilityBudgetReportArray[0];
				utilityChartBudgetTitle = utilityChartBudgetTitle.filter(value => value !== "Month");
				for (var i = 1; i < utilityBudgetReportArray.length; i++) {
					xAxisutilityBudgetChart.push(utilityBudgetReportArray[i][0]);
				}
				$.each(utilityChartBudgetTitle, function(i) {
					var key = utilityChartBudgetTitle[i];
					utilityChartBudgetData[key] = [];
					for (var j = 1; j < utilityBudgetReportArray.length; j++) {
						utilityChartBudgetData[key].push(utilityBudgetReportArray[j][i + 1]);
					}
				});
					...
					if (!(key == 'Budget')) {
						if (key == 'Year - <?php echo ($filters['start_year'] - 1); ?>') {
							utilityBudgetChartSeries.push({
								pointWidth: 15,
								name: key,
								data: utilityChartBudgetData[key],
			...
			<?php } else { ?>
				...
				foreach ($resultkeys as $year => $value) {
					foreach ($value as $key1 => $month) {
						...
						$currentdata_tooltip_array = array();
						$budget_tooltip_array = array();
						$previousdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($filters['start_year'] - 1) . '</strong><br>' . 'Previous Year' . ': ' . number_format($previousdata, 2, '.', ',') . '</br>';
						$currentdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . 'Current Year' . ':' . number_format($currentdata, 2, '.', ',') . '</br>';
						$budgetdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . lang('budget') . ': ' . $budgetdata . '</br>';
				...
				$previousAvgData_tooltip = '<strong>' . lang('average') . '</strong><br>' . 'Previous Year' . ': ' . number_format($previousAvgData, 2, '.', ',') . '</br>';
				$currentAvgData_tooltip  = '<strong>' . lang('average') . '</strong><br>' . 'Current Year	' . ': ' . number_format($currentAvgData, 2, '.', ',') . '</br>';
			...
			Highcharts.chart('utility_report', {
				...
				xAxis: {
					...
					categories: xAxisutilityBudgetChart,
					crosshair: true
				},
				yAxis: {
					min: 0,
				...
				series: utilityBudgetChartSeries
			});
		}
	<?php } ?>
</script>
