...
$utility_changer_list_electricity = array(
        ...
        'electricity_kwh_per_room_night' => array(
                'title'=> lang('electricity')." ".GetSiteUtilityUnitName($site_detail['id'],'electricity')." ".lang('per_room_night'),
                'utility_type'=>'electricity',
                'base_type'=>'unit',
                'formtype'=>'usage_by_room_nights'
            ),
        'electricity_kwh_per_guest' => array(
                'title'=> lang('electricity')." ".GetSiteUtilityUnitName($site_detail['id'],'electricity')." ".lang('per_guest'),
                'utility_type'=>'electricity',
                'base_type'=>'unit',
                'formtype'=>'usage_by_guests'
            ),
        'electricity_kwh_per_built_area' => array(
                'title'=> lang('electricity')." ".GetSiteUtilityUnitName($site_detail['id'],'electricity')." ".lang('per_built_area'),
                'utility_type'=>'electricity',
                'base_type'=>'unit',
                'formtype'=>'usage_by_built_area'
            ),
        'electricity_kwh_per_conditional_area' => array(
                'title'=> lang('electricity')." ".GetSiteUtilityUnitName($site_detail['id'],'electricity')." ".lang('per_cooled_area'),
                'utility_type'=>'electricity',
                'base_type'=>'unit',
                'formtype'=>'usage_by_conditional_area'
            ),
       /* 'electricity_kwh_compare_last_year' => array(
        ...
        'electricity_kwh_budget_forecast' => array(
                'title'=> lang('electricity')." ".GetSiteUtilityUnitName($site_detail['id'],'electricity')." ".lang('vs_budget'),
                'utility_type'=>'electricity',
                'base_type'=>'unit',
                'formtype'=>'usage_by_utility'
            ),
...
<script>
	...
	function drawHighchart() {
		<?php if (!empty($reportdata)) { ?>
			var utilityChartReportArray = [];
			utilityChartReportArray.push(['<?php echo lang("month"); ?>',
				'<?php echo $current_year_text; ?>',
				'<?php echo lang("cdd"); ?>',
				'<?php echo lang("hdd"); ?>', <?php if ($is_occupancy) { ?> '<?php echo lang("occupancy") ?>'
				<?php } ?>,
			]);
			<?php
			$total_sum_current = 0;
			...
			?>
					utilityChartReportArray.push(["<?php echo $monthdata; ?>", <?php echo $currentdata; ?>, <?php echo $cdddata; ?>, <?php echo $hdddata; ?>, <?php if ($is_occupancy) {
																																									echo (isset($occupancydata) && is_finite($occupancydata)) ? $occupancydata : 0;
																																								} ?>]);
			<?php
				}
			}
			...
			var utilityChartSubtitle = utilityChartReportArray[0];
			utilityChartSubtitle = utilityChartSubtitle.filter(value => value !== "Month");
			for (var i = 1; i < utilityChartReportArray.length; i++) {
				xAxisutilityChart.push(utilityChartReportArray[i][0]);
			}
			$.each(utilityChartSubtitle, function(i) {
				var key = utilityChartSubtitle[i];
				utilityChartData[key] = [];
				for (var j = 1; j < utilityChartReportArray.length; j++) {
					utilityChartData[key].push(utilityChartReportArray[j][i + 1]);
				}
			});
			utilityChartSeries = [];
			Object.entries(utilityChartData).forEach(([key, value]) => {
				if (!(key == 'CDD' || key == 'HDD' || key == 'Occupancy')) {
					utilityChartSeries.push({
						pointWidth: 60,
						name: key,
						data: utilityChartData[key],
		...
		Highcharts.chart('utility_report', {
			...
			xAxis: {
				...
				categories: xAxisutilityChart,
				crosshair: true
			},
			<?php if ($is_occupancy && !empty($reportdata)) { ?>
				yAxis: [{
					min: 0,
					title: {
					...
					title: {
						rotation: 270,
						margin: 30,
						text: '<?php echo lang("utility-cost-chart-yaxis-1-occupancy-title"); ?>',
						style: {
							color: Highcharts.getOptions().colors[1],
							fontFamily: 'Arial',
							fontSize: '15px',
			...
			<?php } else { ?>
				yAxis: [{
					min: 0,
					tickPositions: [-1.0, -0.5, 0, 0.5, 1.0],
					title: {
						text: '<?php echo $x_axis_title_value; ?>',
						style: {
							color: Highcharts.getOptions().colors[1],
					...
					title: {
						rotation: 270,
						margin: 30,
						text: '<?php echo lang("utility-cost-chart-yaxis-1-title"); ?>',
						style: {
							color: Highcharts.getOptions().colors[1],
							fontFamily: 'Arial',
							fontSize: '15px',
