...
if (!empty($utility_cost_chart_pre)) {
    ...
    foreach ($resultkeys as $year => $value) {
        ...
        foreach ($value as $key1 => $month) {
            ...
            $pre_data_guest_night_budget = (!empty($utility_cost_chart_pre[$month][$pre_year]['total_guests_budget'])) ? $utility_cost_chart[$month][$year - 1]['total_guests_budget'] : 0;
...
if ($showCostBudgetVariance) {
    ...
    $electricity_tariff_budget = round($currentBudgetActualData["total_electricity_cost_budget"] / $currentBudgetActualData["total_electricity_kwh_budget"], $value_decimal);

    $fuel_oil_tariff_budget = round($currentBudgetActualData["total_fuel_oil_cost_budget"] / $currentBudgetActualData["total_fuel_oil_budget"], $value_decimal);

    $lpg_tariff_budget = round($currentBudgetActualData["total_lpg_cost_budget"] / $currentBudgetActualData["total_lpg_budget"], $value_decimal);

    $natural_gas_tariff_budget = round($currentBudgetActualData["total_natural_gas_cost_budget"] / $currentBudgetActualData["total_natural_gas_budget"], $value_decimal);

    $water_tariff_budget = round($currentBudgetActualData["water_total_consumption_cost_budget"] / $currentBudgetActualData["water_total_consumption_budget"], $value_decimal);

    $district_cooling_tariff_budget = round($currentBudgetActualData["district_cooling_cost_budget"] / $currentBudgetActualData["district_cooling_budget"], $value_decimal);

    $district_heating_tariff_budget = round($currentBudgetActualData["district_heating_cost_budget"] / $currentBudgetActualData["district_heating_budget"], $value_decimal);



    $electricity_tariff_actual = round($currentBudgetActualData["total_electricity_cost_actual"] / $currentBudgetActualData["total_electricity_kwh_actual"], $value_decimal);

    $fuel_oil_tariff_actual = round($currentBudgetActualData["total_fuel_oil_cost_actual"] / $currentBudgetActualData["total_fuel_oil_actual"], $value_decimal);

    $lpg_tariff_actual = round($currentBudgetActualData["total_lpg_cost_actual"] / $currentBudgetActualData["total_lpg_actual"], $value_decimal);

    $natural_gas_tariff_actual = round($currentBudgetActualData["total_natural_gas_cost_actual"] / $currentBudgetActualData["total_natural_gas_actual"], $value_decimal);

    $water_tariff_actual = round($currentBudgetActualData["water_total_consumption_cost_actual"] / $currentBudgetActualData["water_total_consumption_actual"], $value_decimal);

    $district_cooling_tariff_actual = round($currentBudgetActualData["district_cooling_cost_actual"] / $currentBudgetActualData["district_cooling_actual"], $value_decimal);

    $district_heating_tariff_actual = round($currentBudgetActualData["district_heating_cost_actual"] / $currentBudgetActualData["district_heating_actual"], $value_decimal);
    ...
    if ($electricity_tariff_budget != 0) {

        $electricity_tariff_variation = round((($electricity_tariff_actual - $electricity_tariff_budget) * 100) / $electricity_tariff_budget, $percentage_decimal);

    } else {

        $electricity_tariff_variation = 0;

    }



    if ($fuel_oil_tariff_budget != 0) {

        $fuel_oil_tariff_variation = round((($fuel_oil_tariff_actual - $fuel_oil_tariff_budget) * 100) / $fuel_oil_tariff_budget, $percentage_decimal);    

    } else {

        $fuel_oil_tariff_variation = 0;

    }

    

    if ($lpg_tariff_budget != 0) {

        $lpg_tariff_variation = round((($lpg_tariff_actual - $lpg_tariff_budget) * 100) / $lpg_tariff_budget, $percentage_decimal);

    } else {

        $lpg_tariff_variation = 0;

    }



    if ($natural_gas_tariff_budget != 0) {

        $natural_gas_tariff_variation = round((($natural_gas_tariff_actual - $natural_gas_tariff_budget) * 100) / $natural_gas_tariff_budget, $percentage_decimal);

    } else {

        $natural_gas_tariff_variation = 0;
    ...
    if ($water_tariff_budget != 0) {

        $water_tariff_variation = round((($water_tariff_actual - $water_tariff_budget) * 100) / $water_tariff_budget, $percentage_decimal);    

    } else {

        $water_tariff_variation = 0;

    }



    if ($district_cooling_tariff_budget != 0) {

        $district_cooling_tariff_variation = round((($district_cooling_tariff_actual - $district_cooling_tariff_budget) * 100) / $district_cooling_tariff_budget, $percentage_decimal);

    } else {

        $district_cooling_tariff_variation = 0;

    }

    

    if ($district_heating_tariff_budget) {

        $district_heating_tariff_variation = round((($district_heating_tariff_actual - $district_heating_tariff_budget) * 100) / $district_heating_tariff_budget, $percentage_decimal);

    } else {

        $district_heating_tariff_variation = 0;

    }

    

    $electricity_cost_variation = round(($currentBudgetActualData["total_electricity_cost_actual"] - $currentBudgetActualData["total_electricity_cost_budget"]) * 100 / $currentBudgetActualData["total_electricity_cost_budget"], $percentage_decimal);

    $fuel_oil_cost_variation = round(($currentBudgetActualData["total_fuel_oil_cost_actual"] - $currentBudgetActualData["total_fuel_oil_cost_budget"]) * 100 / $currentBudgetActualData["total_fuel_oil_cost_budget"], $percentage_decimal);

    $lpg_cost_variation = round(($currentBudgetActualData["total_lpg_cost_actual"] - $currentBudgetActualData["total_lpg_cost_budget"]) * 100 / $currentBudgetActualData["total_lpg_cost_budget"], $percentage_decimal);

    $natural_gas_cost_variation = round(($currentBudgetActualData["total_natural_gas_cost_actual"] - $currentBudgetActualData["total_natural_gas_cost_budget"]) * 100 / $currentBudgetActualData["total_natural_gas_cost_budget"], $percentage_decimal);

    $water_cost_variation = round(($currentBudgetActualData["water_total_consumption_cost_actual"] - $currentBudgetActualData["water_total_consumption_cost_budget"]) * 100 / $currentBudgetActualData["water_total_consumption_cost_budget"], $percentage_decimal);

    $district_cooling_cost_variation = round(($currentBudgetActualData["district_cooling_cost_actual"] - $currentBudgetActualData["district_cooling_cost_budget"]) * 100 / $currentBudgetActualData["district_cooling_cost_budget"], $percentage_decimal);

    $district_heating_cost_variation = round(($currentBudgetActualData["district_heating_cost_actual"] - $currentBudgetActualData["district_heating_cost_budget"]) * 100 / $currentBudgetActualData["district_heating_cost_budget"], $percentage_decimal);



    $electricity_per_room_night_variation = round(($electricity_per_room_night_actual - $electricity_per_room_night_budget) * 100 / $electricity_per_room_night_budget, $percentage_decimal);

    $fuel_oil_per_room_night_variation = round(($fuel_oil_per_room_night_actual - $fuel_oil_per_room_night_budget) * 100 / $fuel_oil_per_room_night_budget, $percentage_decimal);

    $lpg_per_room_night_variation = round(($lpg_per_room_night_actual - $lpg_per_room_night_budget) * 100 / $lpg_per_room_night_budget, $percentage_decimal);

    $natural_gas_per_room_night_variation = round(($natural_gas_per_room_night_actual - $natural_gas_per_room_night_budget) * 100 / $natural_gas_per_room_night_budget, $percentage_decimal);

    $water_per_room_night_variation = round(($water_per_room_night_actual - $water_per_room_night_budget) * 100 / $water_per_room_night_budget, $percentage_decimal);

    $district_cooling_per_room_night_variation = round(($district_cooling_per_room_night_actual - $district_cooling_per_room_night_budget) * 100 / $district_cooling_per_room_night_budget, $percentage_decimal);

    $district_heating_per_room_night_variation = round(($district_heating_per_room_night_actual - $district_heating_per_room_night_budget) * 100 / $district_heating_per_room_night_budget, $percentage_decimal);
    ...
    $total_cost_variation_percentage = round($total_cost_variation * 100 / $total_cost_budget, $percentage_decimal);
...
include('admin_landing_progress_on_target_report.php');
echo '<br><br>';
