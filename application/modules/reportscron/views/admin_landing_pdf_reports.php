...
if (!empty($utility_cost_chart)) {
    ...
    foreach ($resultkeys as $year => $value) {
        ...
        foreach ($value as $key1 => $month) {
            ...
            $pre_data_water_consumption = (!empty($utility_cost_chart[$month][$prevYear]['water_consumption'])) ? $utility_cost_chart[$month][$prevYear]['water_consumption'] : 0;
            ...
            $pre_data_guest_night_budget = (!empty($utility_cost_chart[$month][$year - 1]['total_guest_night_budget'])) ? $utility_cost_chart[$month][$year - 1]['total_guest_night_budget'] : 0;
            ...
            $data_guest_night_budget = (!empty($utility_cost_chart[$month][$year]['total_guests_budget'])) ? $utility_cost_chart[$month][$year]['total_guests_budget'] : 0;
    ...
    if ($total_sum_pre_data_room_night != 0) {
        ...
        $total_sum_pre_data_electricity_kwh_per_roomnight = 0;

        $total_sum_pre_data_water_liter_per_roomnight = 0;

        $total_sum_pre_data_utility_cost_per_roomnight = 0;
    ...
    if ($site_detail['site_builtup_area'] != 0) {
        ...
        $total_sum_pre_data_electricity_kwh_per_m2 = 0;

        $total_sum_pre_data_utility_cost_per_m2 = 0;

        $total_sum_data_electricity_kwh_per_m2 = 0;

        $total_sum_data_utility_cost_per_m2 = 0;
    ...
    if ($total_sum_data_room_night != 0) {
        ...
        $total_sum_data_electricity_kwh_per_roomnight = 0;

        $total_sum_data_water_liter_per_roomnight = 0;

        $total_sum_data_utility_cost_per_roomnight = 0;
    ...
    if ($total_sum_data_utility_cost_per_roomnight != 0) {
        ...
        $data_utility_cost_per_roomnight_variation = 0;
    ...
    if ($total_sum_data_utility_cost_per_m2 != 0) {

        $data_utility_cost_per_m2_variation = round($data_utility_cost_per_m2_difference * 100 / $total_sum_data_utility_cost_per_m2, $percentage_decimal);    

    } else {

        $data_utility_cost_per_m2_variation = 0;
...
if ($showCostBudgetVariance) {
    ...
    $electricity_tariff_budget = ($currentBudgetActualData["total_electricity_kwh_budget"] != '' && ($currentBudgetActualData["total_electricity_kwh_budget"] != 0)) ? round($currentBudgetActualData["total_electricity_cost_budget"] / $currentBudgetActualData["total_electricity_kwh_budget"], $value_decimal) : 0;

    $fuel_oil_tariff_budget = ($currentBudgetActualData["total_fuel_oil_budget"] != '' && $currentBudgetActualData["total_fuel_oil_budget"] != 0) ? round($currentBudgetActualData["total_fuel_oil_cost_budget"] / $currentBudgetActualData["total_fuel_oil_budget"], $value_decimal) : 0;

    $lpg_tariff_budget = ($currentBudgetActualData["total_lpg_budget"] != '' && $currentBudgetActualData["total_lpg_budget"] != 0) ? round($currentBudgetActualData["total_lpg_cost_budget"] / $currentBudgetActualData["total_lpg_budget"], $value_decimal) : 0;

    $natural_gas_tariff_budget = ($currentBudgetActualData["total_natural_gas_budget"] != '' && $currentBudgetActualData["total_natural_gas_budget"] != 0) ? round($currentBudgetActualData["total_natural_gas_cost_budget"] / $currentBudgetActualData["total_natural_gas_budget"], $value_decimal) : 0;

    $water_tariff_budget = ($currentBudgetActualData["water_total_consumption_budget"] != '' && $currentBudgetActualData["water_total_consumption_budget"] != 0) ? round($currentBudgetActualData["water_total_consumption_cost_budget"] / $currentBudgetActualData["water_total_consumption_budget"], $value_decimal) : 0;

    $district_cooling_tariff_budget = ($currentBudgetActualData["district_cooling_budget"] != '' && $currentBudgetActualData["district_cooling_budget"] != 0) ? round($currentBudgetActualData["district_cooling_cost_budget"] / $currentBudgetActualData["district_cooling_budget"], $value_decimal) : 0;

    $district_heating_tariff_budget = ($currentBudgetActualData["district_heating_budget"] != '' && $currentBudgetActualData["district_heating_budget"] != 0) ? round($currentBudgetActualData["district_heating_cost_budget"] / $currentBudgetActualData["district_heating_budget"], $value_decimal) : 0;



    $electricity_tariff_actual = ($currentBudgetActualData["total_electricity_kwh_actual"] != '' && $currentBudgetActualData["total_electricity_kwh_actual"] != 0) ? round($currentBudgetActualData["total_electricity_cost_actual"] / $currentBudgetActualData["total_electricity_kwh_actual"], $value_decimal) : 0;

    $fuel_oil_tariff_actual = ($currentBudgetActualData["total_fuel_oil_actual"] != '' && $currentBudgetActualData["total_fuel_oil_actual"] != 0) ? round($currentBudgetActualData["total_fuel_oil_cost_actual"] / $currentBudgetActualData["total_fuel_oil_actual"], $value_decimal) : 0;

    $lpg_tariff_actual = ($currentBudgetActualData["total_lpg_actual"] != '' && $currentBudgetActualData["total_lpg_actual"] != 0) ? round($currentBudgetActualData["total_lpg_cost_actual"] / $currentBudgetActualData["total_lpg_actual"], $value_decimal) : 0;

    $natural_gas_tariff_actual = ($currentBudgetActualData["total_natural_gas_actual"] != '' && $currentBudgetActualData["total_natural_gas_actual"] != 0) ? round($currentBudgetActualData["total_natural_gas_cost_actual"] / $currentBudgetActualData["total_natural_gas_actual"], $value_decimal) : 0;

    $water_tariff_actual = ($currentBudgetActualData["water_total_consumption_actual"] != '' && $currentBudgetActualData["water_total_consumption_actual"] != 0) ? round($currentBudgetActualData["water_total_consumption_cost_actual"] / $currentBudgetActualData["water_total_consumption_actual"], $value_decimal) : 0;

    $district_cooling_tariff_actual = ($currentBudgetActualData["district_cooling_actual"] != '' && $currentBudgetActualData["district_cooling_actual"] != 0) ? round($currentBudgetActualData["district_cooling_cost_actual"] / $currentBudgetActualData["district_cooling_actual"], $value_decimal) : 0;

    $district_heating_tariff_actual = ($currentBudgetActualData["district_heating_actual"] != '' && $currentBudgetActualData["district_heating_actual"] != 0) ? round($currentBudgetActualData["district_heating_cost_actual"] / $currentBudgetActualData["district_heating_actual"], $value_decimal) : 0;



    if ($currentBudgetActualData["total_room_night"] != 0) {

        $electricity_per_room_night_actual = round($currentBudgetActualData["total_electricity_kwh_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $fuel_oil_per_room_night_actual = round($currentBudgetActualData["total_fuel_oil_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $lpg_per_room_night_actual = round($currentBudgetActualData["total_lpg_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $natural_gas_per_room_night_actual = round($currentBudgetActualData["total_natural_gas_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $water_per_room_night_actual = round($currentBudgetActualData["water_total_consumption_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $district_cooling_per_room_night_actual = round($currentBudgetActualData["district_cooling_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $district_heating_per_room_night_actual = round($currentBudgetActualData["district_heating_actual"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);



        $electricity_per_room_night_budget = round($currentBudgetActualData["total_electricity_kwh_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $fuel_oil_per_room_night_budget = round($currentBudgetActualData["total_fuel_oil_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);
        ...
        $natural_gas_per_room_night_budget = round($currentBudgetActualData["total_natural_gas_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $water_per_room_night_budget = round($currentBudgetActualData["water_total_consumption_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $district_cooling_per_room_night_budget = round($currentBudgetActualData["district_cooling_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

        $district_heating_per_room_night_budget = round($currentBudgetActualData["district_heating_budget"] / $currentBudgetActualData["total_room_night"], $percentage_decimal);

    } else {

        $electricity_per_room_night_actual = 0;

        $fuel_oil_per_room_night_actual = 0;

        $lpg_per_room_night_actual = 0;

        $natural_gas_per_room_night_actual = 0;

        $water_per_room_night_actual = 0;

        $district_cooling_per_room_night_actual = 0;

        $district_heating_per_room_night_actual = 0;



        $electricity_per_room_night_budget = 0;

        $fuel_oil_per_room_night_budget = 0;

        $lpg_per_room_night_budget = 0;

        $natural_gas_per_room_night_budget = 0;

        $water_per_room_night_budget = 0;

        $district_cooling_per_room_night_budget = 0;

        $district_heating_per_room_night_budget = 0;
    ...
    $fuel_oil_tariff_variation = ($fuel_oil_tariff_budget != '' && $fuel_oil_tariff_budget != 0) ? round((($fuel_oil_tariff_actual - $fuel_oil_tariff_budget) * 100) / $fuel_oil_tariff_budget, $percentage_decimal) : 0;

    $lpg_tariff_variation = ($lpg_tariff_budget != '' && $lpg_tariff_budget != 0) ? round((($lpg_tariff_actual - $lpg_tariff_budget) * 100) / $lpg_tariff_budget, $percentage_decimal) : 0;

    $natural_gas_tariff_variation = ($natural_gas_tariff_budget != '' && $natural_gas_tariff_budget != 0) ? round((($natural_gas_tariff_actual - $natural_gas_tariff_budget) * 100) / $natural_gas_tariff_budget, $percentage_decimal) : 0;

    $water_tariff_variation = ($water_tariff_budget != '' && $water_tariff_budget != 0) ? round((($water_tariff_actual - $water_tariff_budget) * 100) / $water_tariff_budget, $percentage_decimal) : 0;

    $district_cooling_tariff_variation = ($district_cooling_tariff_budget != '' && $district_cooling_tariff_budget != 0) ? round((($district_cooling_tariff_actual - $district_cooling_tariff_budget) * 100) / $district_cooling_tariff_budget, $percentage_decimal) : 0;

    $district_heating_tariff_variation = ($district_heating_tariff_budget != '' && $district_heating_tariff_budget != 0) ? round((($district_heating_tariff_actual - $district_heating_tariff_budget) * 100) / $district_heating_tariff_budget, $percentage_decimal) : 0;



    $electricity_cost_variation = ($currentBudgetActualData["total_electricity_cost_budget"] != '' && $currentBudgetActualData["total_electricity_cost_budget"] != 0) ? round(($currentBudgetActualData["total_electricity_cost_actual"] - $currentBudgetActualData["total_electricity_cost_budget"]) * 100 / $currentBudgetActualData["total_electricity_cost_budget"], $percentage_decimal) : 0;

    $fuel_oil_cost_variation = ($currentBudgetActualData["total_fuel_oil_cost_budget"] != '' && $currentBudgetActualData["total_fuel_oil_cost_budget"] != 0) ? round(($currentBudgetActualData["total_fuel_oil_cost_actual"] - $currentBudgetActualData["total_fuel_oil_cost_budget"]) * 100 / $currentBudgetActualData["total_fuel_oil_cost_budget"], $percentage_decimal) : 0;

    $lpg_cost_variation = ($currentBudgetActualData["total_lpg_cost_budget"] != '' && $currentBudgetActualData["total_lpg_cost_budget"] != 0) ? round(($currentBudgetActualData["total_lpg_cost_actual"] - $currentBudgetActualData["total_lpg_cost_budget"]) * 100 / $currentBudgetActualData["total_lpg_cost_budget"], $percentage_decimal) : 0;

    $natural_gas_cost_variation = ($currentBudgetActualData["total_natural_gas_cost_budget"] != '' && $currentBudgetActualData["total_natural_gas_cost_budget"] != 0) ? round(($currentBudgetActualData["total_natural_gas_cost_actual"] - $currentBudgetActualData["total_natural_gas_cost_budget"]) * 100 / $currentBudgetActualData["total_natural_gas_cost_budget"], $percentage_decimal) : 0;

    $water_cost_variation = ($currentBudgetActualData["water_total_consumption_cost_budget"] != '' && $currentBudgetActualData["water_total_consumption_cost_budget"] != 0) ? round(($currentBudgetActualData["water_total_consumption_cost_actual"] - $currentBudgetActualData["water_total_consumption_cost_budget"]) * 100 / $currentBudgetActualData["water_total_consumption_cost_budget"], $percentage_decimal) : 0;

    $district_cooling_cost_variation = ($currentBudgetActualData["district_cooling_cost_budget"] != '' && $currentBudgetActualData["district_cooling_cost_budget"] != 0) ? round(($currentBudgetActualData["district_cooling_cost_actual"] - $currentBudgetActualData["district_cooling_cost_budget"]) * 100 / $currentBudgetActualData["district_cooling_cost_budget"], $percentage_decimal) : 0;

    $district_heating_cost_variation = ($currentBudgetActualData["district_heating_cost_budget"] != '' && $currentBudgetActualData["district_heating_cost_budget"] != 0) ? round(($currentBudgetActualData["district_heating_cost_actual"] - $currentBudgetActualData["district_heating_cost_budget"]) * 100 / $currentBudgetActualData["district_heating_cost_budget"], $percentage_decimal) : 0;



    $electricity_per_room_night_variation = ($electricity_per_room_night_budget != '' && $electricity_per_room_night_budget != 0) ? round(($electricity_per_room_night_actual - $electricity_per_room_night_budget) * 100 / $electricity_per_room_night_budget, $percentage_decimal) : 0;

    $fuel_oil_per_room_night_variation = ($fuel_oil_per_room_night_budget != '' && $fuel_oil_per_room_night_budget != 0) ? round(($fuel_oil_per_room_night_actual - $fuel_oil_per_room_night_budget) * 100 / $fuel_oil_per_room_night_budget, $percentage_decimal) : 0;

    $lpg_per_room_night_variation = ($lpg_per_room_night_budget != '' && $lpg_per_room_night_budget != 0) ? round(($lpg_per_room_night_actual - $lpg_per_room_night_budget) * 100 / $lpg_per_room_night_budget, $percentage_decimal) : 0;

    $natural_gas_per_room_night_variation = ($natural_gas_per_room_night_budget != '' && $natural_gas_per_room_night_budget != 0) ? round(($natural_gas_per_room_night_actual - $natural_gas_per_room_night_budget) * 100 / $natural_gas_per_room_night_budget, $percentage_decimal) : 0;

    $water_per_room_night_variation = ($water_per_room_night_budget != '' && $water_per_room_night_budget != 0) ? round(($water_per_room_night_actual - $water_per_room_night_budget) * 100 / $water_per_room_night_budget, $percentage_decimal) : 0;

    $district_cooling_per_room_night_variation = ($district_cooling_per_room_night_budget != '' && $district_cooling_per_room_night_budget != 0) ? round(($district_cooling_per_room_night_actual - $district_cooling_per_room_night_budget) * 100 / $district_cooling_per_room_night_budget, $percentage_decimal) : 0;

    $district_heating_per_room_night_variation = ($district_heating_per_room_night_budget != '' && $district_heating_per_room_night_budget != 0) ? round(($district_heating_per_room_night_actual - $district_heating_per_room_night_budget) * 100 / $district_heating_per_room_night_budget, $percentage_decimal) : 0;
