...
class Reports_admin extends Base_Admin_Controller
{
    ...
    public function index($cview = 'index')
    {
        ...
        $electricity_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'electricity');
        $fuel_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'fuel_oil');
        $lpg_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'lpg');
        $natural_gas_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'natural_gas');
        $heating_district_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'district_heating');
        $cooling_district_selected_utility_rate = getUtilityUnitFactorForConversion($site_id, 'district_cooling');
        $electricity_mmbtu_rate = $electricity_mmbtu_rate * $electricity_selected_utility_rate;
        $fuel_mmbtu_rate = $fuel_mmbtu_rate * $fuel_selected_utility_rate;
        $lpg_mmbtu_rate = $lpg_mmbtu_rate * $lpg_selected_utility_rate;
        $natural_gas_mmbtu_rate = $natural_gas_mmbtu_rate * $natural_gas_selected_utility_rate;
        $heating_district_mmbtu_rate = $heating_district_mmbtu_rate * $heating_district_selected_utility_rate;
        $cooling_district_mmbtu_rate = $cooling_district_mmbtu_rate * $cooling_district_selected_utility_rate;
    ...
    public function prepare_budget()
    {
        ...
        foreach ($data['utility_key_array'] as $utility) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alphas[$active_column] . $active_row, round($total));
            // Reset
            $active_row++;
            $active_column = 0;
            /*             * ************ 6th row (Tariff)************* */
