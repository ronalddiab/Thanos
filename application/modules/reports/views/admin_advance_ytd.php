...
$utility_changer_list_water = array(
    'water_liters_per_room_night' => array(
        ...
        'water_cost_budget_forecast' => array(
                'title'=> lang('report_title_water_cost_budget_forecast'),
                'utility_type'=>'water',
                'base_type'=>'unit',
                'formtype'=>'usage_by_utility'
            )
        ...
        <div class="card-wrap">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <div id="utility_report" style="height:500px;">
...
<script>
	...
	<?php if (!empty($reportdata)) {  ?>
		...
		function drawHighchart() {
			<?php if ($data['time_type'] == 'advance_select_avg_ytd') { ?>
				var utilityYTD_PreviousReportArray = [];
				utilityYTD_PreviousReportArray.push(['<?php echo lang("month"); ?>',
					'<?php echo $previous_year_text; ?>',
					'<?php echo $current_year_text; ?>',
					<?php if ($is_occupancy) { ?> '<?php echo ($filters['start_year'] - 1) . " - " . lang("occupancy"); ?>',
						'<?php echo $filters['start_year'] . " - " . lang("occupancy"); ?>'
					<?php } ?>,
				]);
				<?php
				$total_sum_current = 0;
				...
				foreach ($resultkeys as $year => $value) {
					foreach ($value as $key1 => $month) {
						...
						$previousdata = (!empty($reportdata[$month][$year - 1][$filters['utility_type']]) && is_numeric($reportdata[$month][$year - 1][$filters['utility_type']]) && is_finite($reportdata[$month][$year - 1][$filters['utility_type']])) ? $reportdata[$month][$year - 1][$filters['utility_type']] : 0;
						$currentdata = (!empty($reportdata[$month][$year][$filters['utility_type']]) && is_numeric($reportdata[$month][$year][$filters['utility_type']]) && is_finite($reportdata[$month][$year][$filters['utility_type']])) ? $reportdata[$month][$year][$filters['utility_type']] : 0;
						...
						$previousoccupancydata = (!empty($reportdata[$month][$year - 1]['occupancy']) && is_numeric($reportdata[$month][$year - 1]['occupancy']) && is_finite($reportdata[$month][$year - 1]['occupancy'])) ? $reportdata[$month][$year - 1]['occupancy'] : 0;
						...
						$previousdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($year - 1) . '</strong><br>' . 'Previous Year' . ': ' . $previousdata . '</br>';
						$currentdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $year . '</strong><br>' . 'Current Year' . ':' . $currentdata . '<br>';
						$occupancydata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $year . '</strong><br>' . lang('occupancy') . ': ' . $occupancydata;
						$previousoccupancydata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($year - 1) . '</strong><br>' . lang('occupancy') . ': ' . $previousoccupancydata;
				...
				?>
						...
						for ($i = 1; $i <= count($fullmontharray); $i++) {
							$previousArray[] = $montharray[$i] . " " . ($filters['start_year'] - 1);
							$currentArray[] = $montharray[$i] . " " . ($filters['start_year']);
							$occupancyArray[] = $montharray[$i] . " " . ($filters['start_year']);
							$previousoccupancyArray[] = $montharray[$i] . " " . ($filters['start_year']);
						}
						$previousArray[] = "Average";
						$currentArray[] = "Average";
						?>
						utilityYTD_PreviousReportArray.push(["<?php echo $monthdata; ?>", <?php echo $previousdata; ?>, <?php echo $currentdata; ?><?php if ($is_occupancy) {
																																						echo ',' . $previousoccupancydata;
																																						echo ',' . $occupancydata;
																																					} ?>]);
				<?php
					}
				...
				$previousAvgData_tooltip = '<strong>' . lang('average') . '</strong><br>' . 'Previous Year' . ': ' . $previousAvgData . '</br>';
				$currentAvgData_tooltip  = '<strong>' . lang('average') . '</strong><br>' . 'Current Year' . ': ' . $currentAvgData . '</br>';

				$previousdata_tooltip_array[0][] = $previousAvgData_tooltip;
				$currentdata_tooltip_array[0][] = $currentAvgData_tooltip;
				?>
				utilityYTD_PreviousReportArray.push(["<?php echo lang('average'); ?>", <?php echo $previousAvgData; ?>, <?php echo $currentAvgData; ?><?php if ($is_occupancy) {
																																							echo ',' . $previousoccupancyAvgData;
				...
				var utilityChartCommonData = [];
				var xAxisutilityCommonChart = [];
				var utilityChartYTD_PreviousTitle = utilityYTD_PreviousReportArray[0];
				utilityChartYTD_PreviousTitle = utilityChartYTD_PreviousTitle.filter(value => value !== "Month");
				for (var i = 1; i < utilityYTD_PreviousReportArray.length; i++) {
					xAxisutilityCommonChart.push(utilityYTD_PreviousReportArray[i][0]);
				}
				$.each(utilityChartYTD_PreviousTitle, function(i) {
					var key = utilityChartYTD_PreviousTitle[i];
					utilityChartCommonData[key] = [];
					for (var j = 1; j < utilityYTD_PreviousReportArray.length; j++) {
						utilityChartCommonData[key].push(utilityYTD_PreviousReportArray[j][i + 1]);
					}
				});
				utilityCommonChartSeries = [];
				Object.entries(utilityChartCommonData).forEach(([key, value]) => {
					if (!(key == '<?php echo (date('Y') - 1) ?> - Occupancy' || key == '<?php echo date('Y') ?> - Occupancy')) {
						if (key == 'Previous Year') {
							utilityCommonChartSeries.push({
								pointWidth: 15,
								name: key,
								data: utilityChartCommonData[key],
						...
						if (key == '<?php echo (date('Y') - 1) ?> - Occupancy') {
							utilityCommonChartSeries.push({
								type: 'spline',
								name: key,
								yAxis: 1,
						...
						if (key == '<?php echo date('Y') ?> - Occupancy') {
							utilityCommonChartSeries.push({
								type: 'spline',
								name: key,
								yAxis: 1,
			...
			<?php } else { ?>
				var utilityChooseYearReportArray = [];
				utilityChooseYearReportArray.push(['<?php echo lang("month"); ?>',
					'<?php echo $previous_year_text; ?>',
					'<?php echo $current_year_text; ?>',
					<?php if ($is_occupancy) { ?> '<?php echo ($filters['start_year'] - 1) . " - " . lang("occupancy"); ?>',
						'<?php echo $filters['start_year'] . " - " . lang("occupancy"); ?>'
				...
				foreach ($resultkeys as $year => $value) {
					foreach ($value as $key1 => $month) {
						...
						$occupancydata_tooltip_array = array();
						$previousoccupancydata_tooltip_array = array();
						$previousdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($filters['start_year'] - 1) . '</strong><br>' . 'Year - ' . ($filters['start_year'] - 1) . ': ' . $previousdata . '</br>';
						$currentdata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . 'Year - ' . $filters['start_year'] . ':' . $currentdata . '<br>';
						$occupancydata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . $filters['start_year'] . '</strong><br>' . lang('occupancy') . ': ' . $occupancydata;
						$previousoccupancydata_tooltip[] = '<strong>' . $montharray[$month] . ' ' . ($filters['start_year'] - 1) . '</strong><br>' . lang('occupancy') . ': ' . $previousoccupancydata;
				...
				?>
						...
						$currentArray[] = "Average";
						?>
						utilityChooseYearReportArray.push(["<?php echo $monthdata; ?>", <?php echo $previousdata; ?>, <?php echo $currentdata; ?><?php if ($is_occupancy) {
																																						echo ',' . $previousoccupancydata;
																																						echo ',' . $occupancydata;
																																					} ?>]);
				<?php
				...
				$previousAvgData_tooltip = '<strong>' . lang('average') . '</strong><br>' . 'Year - ' . ($filters['start_year'] - 1) . ': ' . $previousAvgData . '</br>';
				$currentAvgData_tooltip  = '<strong>' . lang('average') . '</strong><br>' . 'Year - ' . $filters['start_year'] . ': ' . $currentAvgData . '</br>';

				$previousdata_tooltip_array[0][] = $previousAvgData_tooltip;
				$currentdata_tooltip_array[0][] = $currentAvgData_tooltip;
				?>
				utilityChooseYearReportArray.push(["<?php echo lang('average'); ?>", <?php echo $previousAvgData; ?>, <?php echo $currentAvgData; ?><?php if ($is_occupancy) {
																																						echo ',' . $previousoccupancyAvgData;
																																						echo ',' . $occupancyAvgData;
																																					} ?>]);
				var previoustooltipArray = <?php echo json_encode($previousdata_tooltip_array[0]); ?>;
				...
				var utilityChartCommonData = [];
				var xAxisutilityCommonChart = [];
				var utilityChartYTD_PreviousTitle = utilityChooseYearReportArray[0];
				utilityChartYTD_PreviousTitle = utilityChartYTD_PreviousTitle.filter(value => value !== "Month");
				for (var i = 1; i < utilityChooseYearReportArray.length; i++) {
					xAxisutilityCommonChart.push(utilityChooseYearReportArray[i][0]);
				}
				$.each(utilityChartYTD_PreviousTitle, function(i) {
					var key = utilityChartYTD_PreviousTitle[i];
					utilityChartCommonData[key] = [];
					for (var j = 1; j < utilityChooseYearReportArray.length; j++) {
						utilityChartCommonData[key].push(utilityChooseYearReportArray[j][i + 1]);
					}
				});
				utilityCommonChartSeries = [];
				Object.entries(utilityChartCommonData).forEach(([key, value]) => {
					if (!(key == '<?php echo ($filters['start_year'] - 1) ?> - Occupancy' || key == '<?php echo $filters['start_year'] ?> - Occupancy')) {
						...
						if (key == '<?php echo ($filters['start_year'] - 1); ?> - Occupancy') {
							utilityCommonChartSeries.push({
								type: 'spline',
								name: key,
								yAxis: 1,
								...
								marker: {
									symbol: 'square',
									lineWidth: 2,
									lineColor: '<?php echo $this->_ci->config->config['chart_legend_colors'][($filters['start_year'] - 1)] ?>',
									fillColor: '<?php echo $this->_ci->config->config['chart_legend_colors'][($filters['start_year'] - 1)] ?>'
								},
								color: '<?php echo $this->_ci->config->config['chart_legend_colors'][($filters['start_year'] - 1)] ?>'
							}, );
						}
						if (key == '<?php echo $filters['start_year'] ?> - Occupancy') {
							utilityCommonChartSeries.push({
								type: 'spline',
								name: key,
								yAxis: 1,
								...
								marker: {
									symbol: 'square',
									lineWidth: 2,
									lineColor: '<?php echo $this->_ci->config->config['chart_legend_colors'][$filters['start_year']] ?>',
									fillColor: '<?php echo $this->_ci->config->config['chart_legend_colors'][$filters['start_year']] ?>'
								},
								color: '<?php echo $this->_ci->config->config['chart_legend_colors'][$filters['start_year']] ?>'
							}, );
						}
					}
				});
			...
			Highcharts.chart('utility_report', {
				...
				xAxis: {
					...
					categories: xAxisutilityCommonChart,
					crosshair: true
				},
				<?php if ($is_occupancy) { ?>
					yAxis: [{
					...
					tooltip: {
						formatter: function() {
							...
							for (var i = 0; i <= this.point.x; i++) {
								previoustooltipdata = previoustooltipArray[i];
								currenttooltipdata = currenttooltipArray[i];
				...
				series: utilityCommonChartSeries
			});
		}
	<?php } ?>
</script>
