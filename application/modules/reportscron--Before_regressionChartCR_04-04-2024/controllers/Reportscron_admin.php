...

{
	...
	if (!empty($this->input->post())) {
	    ...
	    $data['monthly']['columnChartImg'] = $postData['columnChartImg'];//$postdata['columnChartImg_monthly_month'];
	    ...
	    $progressOnTarget = $filtersBaseline = array();
			if(date('n') == 1) {
				$filters['Progress_start_year'] = date('Y') - 1;
				$filters['Progress_previous_year'] = date('Y') - 2;
			} else {
				$filters['Progress_start_year'] = date('Y');
				$filters['Progress_previous_year'] = date('Y') - 1;
			}
	    $this->reports_model->site_id = $site_id;
	    $progressOnTargetData = $this->reports_model->monthlyUtilityProgressOnTarget($filters);
	    $filtersBaseline['Progress_start_year'] = $site_detail['baseline_regression_year'];
	    ...
	    $data['monthly']['progressOnTarget'] = $progressOnTarget;
	    $site_detials =  $this->sites_model->get_site_detail_custom($site_id);
	    $data['monthly']['site_detials'] = $site_detials;
	    $data['monthly']['carbon'] = $this->getCarbonRecords($site_id, $site_detials);
		...
		if (!empty($site_ids)) {
			...
			foreach ($data['sites'] as $key => $site_detials) {
			    ...
			    // Default notifications for electricity,water and Thermal (Fuel + LPG + Natural Gaz + District Energy)
							...
							$electricity_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'electricity');;
							$fuel_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'fuel_oil');;
							$lpg_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'lpg');
							$natural_gas_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'natural_gas');
							$heating_district_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'district_heating');
							$cooling_district_mmbtu_rate = getUtilityUnitFactorForConversion($siteId, 'district_cooling');
	...
	foreach ($users as $key => $user) {
	    /* if ($key >= 0 && $key <= 100) { // for demo user batch dhaval only
		echo '<script>

		    console.log("BATCH '.$key.' User Id '.$user['id'].'");

		</script>';
	    // if ($user['id'] == 1) { // for demo user dhaval only
	    if ($user['role_id'] == 1) {
				...
				foreach ($data['sites'] as $key => $site_detials) {
					...
					if ($data['sites'][$key]['data']['actionplan']['actionPlanCompleteCounts'] > 0) {
					    ...
					    $data['sites'][$key]['data']['actionplan']['actionPlanCompletePercentage'] = (($data['sites'][$key]['data']['actionplan']['actionPlanCompleteCounts'] * 100) / $data['sites'][$key]['data']['actionplan']['actionPlanCounts']);
									...
									$data['sites'][$key]['data']['kpi'] = $this->getCarbonRecords($site_id, $site_detials);
 ...
 */
	    ...
	    // } // for demo user batch dhaval only
	}
	...
	foreach ($users as $key => $user) {
		...
		if (!empty($user['sites']) && in_array('upper_management', $user['reports'])) {
			...
			if (!empty($site_ids)) {
				...
				foreach ($data['sites'] as $key => $site_detials) {
				    $site_id = $site_detials['id'];
				    $site_detials = $this->sites_model->getEmissionFactorYearly($site_detials);
				    if ($site_id != '') {
				    // Get Actionplans for site
				    $actionPlans = $this->projects_model->getAllActionPlansBySiteId($site_id);
				    $data['sites'][$key]['data']['actionplan']['actionPlanCounts'] = 0;
				    ...
				    $filters_notification['cmonth'] = (int) date("m", strtotime(date('Y-m') . " -1 months"));
				    $filters_notification['cyear'] = date("Y", strtotime(date('Y-m') . " -1 months"));
				    $this->dashboard_model->site_id = $site_detials['id'];
					...
					foreach ($utility_cost_chart_results as $key1 => $value) {
					    ...
					    $data['utility_cost_chart'][$value['month_id']][$value['year_id']]['room_night'] = $value['total_room_night'];
					    $days_of_month = cal_days_in_month(CAL_GREGORIAN, $value['month_id'], $value['year_id']);
												$data['sites'][$key]['utility_cost_chart']['occupancy'] = (($value['rooms_keys'] * $days_of_month) != 0) ? (($value['total_room_night'] / ($value['rooms_keys'] * $days_of_month)) * 100) : 0;
					}
				...
				switch ($time_type) {
					...
					if (!empty($results)) {
						...
						$results[$key]['heating_district_cost'] = $result['heating_district_cost'] + $result['district_heating_fixed_cost'];
												...
												$result['occupancy'] = (($result['rooms_keys'] * $days_of_month) != 0) ? (($result['total_room_night'] / ($result['rooms_keys'] * $days_of_month)) * 100) : 0;
			...
			foreach ($data['sites'] as $key => $site) {
			    ...
			    $progressOnTargetData = $this->reports_model->monthlyUtilityProgressOnTarget($filtersProgress);
			    ...
			    $postData['site']['data']['progressOnTarget'] = isset($progressOnTarget) ? $progressOnTarget : [];
			    }
			    $html .= $this->load->view('admin_sites_pdf', $postData, true);
			    if ($margin_flag % 2 == 1 || $site_count == ($margin_flag + 1)) {
				$html .= '</div>';
			...
			$this->mailer->mail->addAttachment($file_name, "Utilities Dashboard Report - ".$regionNamePdf.".pdf");
		    }
