...

{
...

    {
        ...
        return array(
            ...
            array(
                'actions' => array('index', 'edit', 'add', 'action', 'delete', 'save', 'view_data', 'set_user_theme', 'set_notification', 'pdf', 'cron_settings','get_sites', 'waste', 'emission', 'delete_emission_image', 'view_area_history','residence','export_waste','replicate_residence','export_utility','permission'),
                'users'   => array('@'),
   ...
   public function get_sites()
    ...
    public function export_waste(){
        ...
        foreach ($tabData as $keyRow1 => $valueRow1) {
            ...
            if (strpos($row1Name, '_#') !== false) { $data = explode('_#',$row1Name); $row1Name = isset($data[2]) ? $data[2] : $data[0]; }
            ...
            if(!isset($valueRow1) && empty($valueRow1)) {
                ...
                foreach ($valueRow1 as $keyRow2 => $valueRow2) {
                    ...
                    if (strpos($row2Name, '_#') !== false) { $data = explode('_#',$row2Name); $row2Name = isset($data[2]) ? $data[2] : $data[0]; }
            ...
            if($index !== 0) {
                ...
                foreach ($sites as $keyWasteSetting => $wasteSetting) {
                    ...
                    foreach ($siteWasteUtility as $keyWasteData => $wasteData) {
                        if($wasteData['month_id'] <= 12 && $wasteData['month_id'] >= 1) {
                            ...
                            $siteWasteUtilityData[$wasteData['site_id']][$wasteData['month_id']][$wasteData['year_id']] = $wasteData;
                            $indexWasteData++;
                        }                       
                    }
                }
                            ...
                            if(empty($val) || $val == 0 || !isset($val)) {
                                ...
                                if($index == 0) {
                                    ...
                                    if($sites[$keyMain]['monthly_tracking_'.$keyName] == 1) {
                                        ...
                                        $val = (isset($amount) && !empty($amount) && ($amount != 0)) ? $amount.'-'.html_entity_decode($unit_measures[$val]) : '0';
                                    }
                                } else if($index == 1) {
    ...
    public function export_utility(){
        ...
        $columns['total_kwh'] = 'Electricity: Purchased Electricity';
        ...
        foreach ($UtilityData as $utl) {
            foreach ($utl as $key => $val) {
                if (!empty($val) && ($key != "site_location_name") &&  (strpos( $key, '_unit' ) == false) && ($key != "attribute")  && ($key != "local_currency")) {
                    $val = round($val, 4);
                }
