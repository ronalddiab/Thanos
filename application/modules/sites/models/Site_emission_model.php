...
class Site_Emission_model extends Base_Model
{
    ...
    public function get_site_emission_model_detail_by_siteId()
    {
        ...
        if (isset($this->year_ids) && is_array($this->year_ids) && !empty($this->year_ids)) {
            $this->db->where_in('s.year_id', $this->year_ids);
        } elseif (isset($this->year_id) && !empty($this->year_id)) {
            // Fallback to single year filter
            $this->db->where('s.year_id', (int) $this->year_id);
        }
