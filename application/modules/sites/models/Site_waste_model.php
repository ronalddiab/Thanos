...
class Site_Waste_model extends Base_Model
{
    ...
    /**
     * unit_measure_dropdown id => factor to convert stored unit_measure to kg
     */
    private function getUnitMeasureToKgFactors()
    {
        return [
            1  => 1000,      // Metric Ton (MT)
            2  => 907.185,   // Short Ton (tn)
            5  => 1,         // Litres (default; item-specific overrides below)
            17 => 208,       // Grease Drum (55gal)
            18 => 1,         // Kilograms
            19 => 0.453592,  // Pounds
        ];
    }
    ...
    /**
     * Litre (id 5) → kg by waste stream.
     * Kitchen grease: 1 L = 0.00085 MT = 0.85 kg
     * Liquid food waste: 1 L = 0.0009188 MT = 0.9188 kg
     */
    private function getLitreToKgFactorByItem($wasteItem)
    {
        $litreToKgByItem = [
            'kitchen_grease'    => 0.85,
            'liquid_food_waste' => 0.9188,
        ];
        return $litreToKgByItem[$wasteItem] ?? null;
    }

    private function getUnitMeasureToKgFactor($unitId, $wasteItem = null)
    {
        $unitId = (int) $unitId;

        if ($unitId === 5 && $wasteItem !== null) {
            $itemFactor = $this->getLitreToKgFactorByItem($wasteItem);
            if ($itemFactor !== null) {
                return $itemFactor;
            }
        }

        $factors = $this->getUnitMeasureToKgFactors();
        return $factors[$unitId] ?? 1;
    }
    
    public function get_site_waste_model_detail_by_siteId_userId()
    {
        $year_id  = (!empty($this->year_id)) ? (int) $this->year_id : NULL;
        $month_id = (!empty($this->month_id)) ? (int) $this->month_id : NULL;
    ...
    public function get_site_waste_utility_data($site_id, $currYear = 0, $currMonth = 0, $columnChecked = []) 
    {
        if (empty($columnChecked)) {
            ...
            $this->db->where('s.month_id is NOT NULL', null, false);
            if (!empty($currYear) && isset($currYear)) {
         ...
         * --------------------------------------------------------------------------
         *  COLUMN LOGIC:
         *  - $value = 'plastic' → select columns ending with '_plastic'
         *  - Dynamic: find actual column names from DB
         * --------------------------------------------------------------------------
         */
        ...
        if (!empty($columnChecked)) {
            ...
            $sumPartsUnitMeasuresWasteToEnergy = [];
            $sumPartsTotalWaste = [];

            // Per-column unit from YTD settings row (unit_measure_dropdown_{colName})
            $unitsByItem = [];
            $unitSelectParts = [];
            foreach ($columnChecked as $item) {
                $unitSelectParts[] = "s.unit_measure_dropdown_{$item} AS unit_{$item}";
            }
            $this->db->reset_query();
            $this->db->select(implode(', ', $unitSelectParts), false);
            $this->db->from($this->_table . ' AS s');
            $this->db->where('s.site_id', $site_id);
            $this->db->where('s.deleted_at', null);
            $this->db->where('s.deleted_by', null);
            ...
            $this->db->limit(1);
            $unitRow = $this->db->get()->row_array();
            if (!empty($unitRow)) {
                foreach ($columnChecked as $item) {
                    if (!empty($unitRow["unit_{$item}"])) {
                        $unitsByItem[$item] = (int) $unitRow["unit_{$item}"];
                    }
                }
            }
            $unitMeasureSumExpr = function ($colName) use ($unitsByItem) {
                $unitMeasureColumn = 'unit_measure_' . $colName;
                $expr = "COALESCE(s.`{$unitMeasureColumn}`, 0)";
                $unitId = $unitsByItem[$colName] ?? null;
                if ($unitId !== null) {
                    $factor = $this->getUnitMeasureToKgFactor($unitId, $colName);
                    if ($factor != 1) {
                        $expr = "({$expr} * {$factor})";
                    }
                }
            ...
            foreach ($grouping as $groupKey => $groupColumns) {
                $sumPartsUnitMeasure = [];
                foreach ($groupColumns as $colName) {
                    $sumPartsUnitMeasure[] = $unitMeasureSumExpr($colName);
                }
                if (!empty($sumPartsUnitMeasure)) {
                    // Generate SUM() for the group
                    $selectedColsUnitMeasure[] = implode(" + ", $sumPartsUnitMeasure). " AS `{$groupKey}_unit_measure`";
                }
            }

            foreach ($columnChecked as $colName) {
                
                $destinationCol = "typical_destination_" . $colName;
                // Total waste (default YTD): exclude typical_destination 7 — same as getWasteYTDByDestinationAndCurrMonth default filter
                $sqlTotal = "
                    SELECT 1
                    FROM {$this->_table} s
                ...
                // Waste to Energy only — typical_destination 5 per waste settings
                $sqlWasteToEnergy = "
                    SELECT 1
                    FROM {$this->_table} s
                    WHERE s.deleted_at IS NULL
                ...
                if (isset($queryResponseWasteToEnergy) && !empty($queryResponseWasteToEnergy)) {
                    $sumPartsUnitMeasuresWasteToEnergy[] = $unitMeasureSumExpr($colName);
                }
            }
            ...
            if (!empty($sumPartsUnitMeasuresRecyclings)) {
                $this->db->select(
                    implode(' + ', $sumPartsUnitMeasuresRecyclings) . ' AS `diverted_recycling_unit_measure`',
                    false
                );
            }
            if (!empty($sumPartsTotalWaste)) {
                $this->db->select(
                    implode(' + ', $sumPartsTotalWaste) . ' AS `total_waste_unit_measure`',
                    false
                );
            }
            if (!empty($sumPartsUnitMeasuresWTE)) {
                $this->db->select(
                    implode(' + ', $sumPartsUnitMeasuresWTE) . ' AS `recycling_wte_unit_measure`',
                    false
                );
            }
            if (!empty($sumPartsUnitMeasuresWasteToEnergy)) {
                $this->db->select(
                    implode(' + ', $sumPartsUnitMeasuresWasteToEnergy) . ' AS `wte_unit_measure`',
                    false
                );
            }
            $this->db->from($this->_table . ' AS s');
    ...
    function insert_site_waste() {
        ...
        if(isset($this->is_check_bottles_cans) && empty($this->month_id) && empty($this->year_id)) {
         ...
         if(isset($this->typical_destination_wastetoenergy)) {
            $data_array['typical_destination_wastetoenergy'] = $this->typical_destination_wastetoenergy;
        } else {
            $data_array['typical_destination_wastetoenergy'] = 0;
        }

        if(isset($this->unit_measure_dropdown_wastetoenergy)) {
            $data_array['unit_measure_dropdown_wastetoenergy'] = $this->unit_measure_dropdown_wastetoenergy;
        } else {
            $data_array['unit_measure_dropdown_wastetoenergy'] = 0;
        }

        if(isset($this->source_wastetoenergy)) {
            $data_array['source_wastetoenergy'] = $this->source_wastetoenergy;
        } else {
            $data_array['source_wastetoenergy'] = 0;
        }

        if(isset($this->monthly_tracking_wastetoenergy)) {
            $data_array['monthly_tracking_wastetoenergy'] = $this->monthly_tracking_wastetoenergy;
        } else {
            $data_array['monthly_tracking_wastetoenergy'] = 0;
        }

        if(isset($this->unit_measure_wastetoenergy)) {
            $data_array['unit_measure_wastetoenergy'] = $this->unit_measure_wastetoenergy;
        }

        if(isset($this->disposal_cost_wastetoenergy)) {
            $data_array['disposal_cost_wastetoenergy'] = $this->disposal_cost_wastetoenergy;
        }

        if(isset($this->total_wastetoenergy)) {
            $data_array['total_wastetoenergy'] = $this->total_wastetoenergy;
        }

        if(isset($this->is_check_wastetoenergy) && empty($this->month_id) && empty($this->year_id)) {
            $data_array['is_check_wastetoenergy'] = $this->is_check_wastetoenergy;
        }
        ...
        if (isset($this->unit_measure_hazardous_waste) && !empty($this->unit_measure_hazardous_waste)) {
            ...
            $data_array['unit_measure_hazardous_waste'] = 0;
        }
        ...
        if (isset($this->disposal_cost_hazardous_waste) && !empty($this->disposal_cost_hazardous_waste)) {
            ...
            $data_array['disposal_cost_hazardous_waste'] = 0;
        }
        ...
        if (isset($this->total_hazardous_waste) && !empty($this->total_hazardous_waste)) {
            ...
            $data_array['total_hazardous_waste'] = 0;
        }
        ...
        if (isset($this->is_check_hazardous_waste) && empty($this->month_id) && empty($this->year_id)) {
            ...
            $data_array['is_check_hazardous_waste'] = 0;
        }
        ...
        if (isset($this->unit_measure_other_materials) && !empty($this->unit_measure_other_materials)) {
            ...
            $data_array['unit_measure_other_materials'] = 0;
        }
        ...
        if (isset($this->disposal_cost_other_materials) && !empty($this->disposal_cost_other_materials)) {
            ...
            $data_array['disposal_cost_other_materials'] = 0;
        }
        ...
        if (isset($this->total_other_materials) && !empty($this->total_other_materials)) {
            ...
            $data_array['total_other_materials'] = 0;
        }
        ...
        if (isset($this->is_check_other_materials) && empty($this->month_id) && empty($this->year_id)) {
            ...
            $data_array['is_check_other_materials'] = 0;
        }
        ...
        if(isset($this->unit_measure_hazardous_and_universal_waste)) {
            ...
            $data_array['unit_measure_hazardous_and_universal_waste'] = 0;
        }
        ...
        if(isset($this->disposal_cost_hazardous_and_universal_waste)) {
            ...
            $data_array['disposal_cost_hazardous_and_universal_waste'] = 0;
        }
        ...
        if(isset($this->total_hazardous_and_universal_waste)) {
            ...
            $data_array['total_hazardous_and_universal_waste'] = 0;
        }
        ...
        if(isset($this->is_check_hazardous_and_universal_waste) && empty($this->month_id) && empty($this->year_id)) {
            ...
            $data_array['is_check_hazardous_and_universal_waste'] = 0;
        }
        ...
        if(isset($this->is_check_medical_waste) && empty($this->month_id) && empty($this->year_id)) {
            ...
            $data_array['is_check_medical_waste'] = 0;
        }
        ...
        if(empty($dataAlreadyExist)) {
            ...
            if(!empty($this->year_id) && !empty($this->month_id)) {
                $this->db->where([
                    'year_id' => $this->year_id,
                    'month_id' => $this->month_id
                ]);
            } else {
                $this->db->where('year_id IS NULL', null, false);
                $this->db->where('month_id IS NULL', null, false);
            }
    ...
    public function getLastMonthForSite($site_id) {
        ...
        $this->db->where('site_id', $site_id);
        $this->db->where('month_id is NOT NULL', NULL, FALSE);
        $this->db->where('site_id is NOT NULL', NULL, FALSE);
        $this->db->where('year_id is NOT NULL', NULL, FALSE);
    ...
    public function getCheckedColumnNames()
    {
        $columns = $this->db->query("
            ...
            AND COLUMN_NAME LIKE 'is_check_%'
        ")->result_array();
        ...
        $selects = array_map(function ($c) {
            return "MAX({$c['COLUMN_NAME']}) AS {$c['COLUMN_NAME']}";
        }, $columns);
        ...
        foreach ($row as $col => $val) {
            if ((int)$val === 1) {
                $checked[] = str_replace('is_check_', '', $col);
            }
        }
...
/**
     * $currYear/$currMonth — optional YTD end (if null the function uses rule:
     *   if now month == 1 => YTD end = previous year, month=12
     *   else => YTD end = current year, month = current month)
     */
    public function getWasteYTDByDestinationAndCurrMonth($siteData, $wasteDestination = '', $currYear = null, $currMonth = null) {
        $siteId = $this->site_id = $siteData['id'];
        $targetYear = $siteData['baseline_regression_year'];
        ...
        foreach ($groupedParents as $groupColumns) {
            foreach ($groupColumns as $colName) {
                if(in_array($colName, $columnChecked)) {
                    ...
                    $this->db->from($this->_table . ' AS s');
                    $this->db->where('s.site_id', $siteId);
                    $this->db->where('s.deleted_at', null);
                    $this->db->where('s.deleted_by', null);

                    $this->db->group_start();
                        $this->db->where_in('s.year_id', [(int)$currYear, (int)$targetYear]);
                        $this->db->or_where('s.year_id IS NULL', null, false);
                    $this->db->group_end();

                    $this->db->group_start();
                        $this->db->where('s.month_id <=', (int)$currMonth);
                        $this->db->or_where('s.month_id IS NULL', null, false);
                    $this->db->group_end();

                    $this->db->group_by('s.site_id, s.year_id');
                    $this->db->order_by('s.year_id IS NULL', 'DESC', false);
                    ...
                    foreach ($result as $row) {
                        ...
                        if (
                            is_null($row['year_id']) &&
                            is_null($row['month_id'])
                        ) {
                            $multiplyFactor = $this->getUnitMeasureToKgFactor($row['unit'] ?? null, $wasteItem);
                            $itemwiseTotalWasteArray[$wasteItem]['setting'] = $row;
                            continue;
                        }

                        // YEAR DATA
                        $year = (int)$row['year_id'];
                        $row['unit_measure'] *= ($multiplyFactor ?? 1);
                        unset($row['month_id'], $row['unit'], $row['typical_destination']);
            ...
            switch ($wasteDestination) {
                case 'recycling':
                    $itemwiseTotalWasteArray = array_filter($itemwiseTotalWasteArray, function ($item) {
                        $excludeUKLandfillWhenRecycling = [1,7];
                        return isset($item['setting']['typical_destination'])
                            && !in_array((int)$item['setting']['typical_destination'], $excludeUKLandfillWhenRecycling, true);
                    });
                    break;
                ...
                case 'landfill':
                    $itemwiseTotalWasteArray = array_filter($itemwiseTotalWasteArray, function ($item) {
                        return isset($item['setting']['typical_destination'])
                            && (int)$item['setting']['typical_destination'] === 1;
                    });
                    break;
                ...
                default:
                    $itemwiseTotalWasteArray = array_filter($itemwiseTotalWasteArray, function ($item) {
                        $exclude = [];// no exclusions for total waste, all streams count - Previously Unknown excluded
                        return isset($item['setting']['typical_destination'])
                            && !in_array((int)$item['setting']['typical_destination'], $exclude, true);
                    });
                    break;
            }
        }
        $YTDTotal = [];
        foreach ($itemwiseTotalWasteArray as $wasteData) {
            foreach ($wasteData as $key => $row) {
                if ($key === 'setting') {
                    continue;
                }

                if (isset($row['year_id'], $row['unit_measure'])) {
                    $year = (int) $row['year_id'];
                    $YTDTotal[$year] = ($YTDTotal[$year] ?? 0) + (float) $row['unit_measure'];
                }
            }
        }
        ksort($YTDTotal);
    ...
    public function getWasteReportData($site_id, $wasteData, $currYear, $currMonth) {
        ...
        // Per-category rows use grouped parent keys from groupSelectedItems (same as get_site_waste_utility_data).
        // Unit measures are already converted to kg in get_site_waste_utility_data.
        // Recyclables and total waste are aligned with getWasteYTDByDestinationAndCurrMonth (diverted recycling + excl. dest 7).
        $categories = [
            'organic'      => 'biodegradable',
            'hazardous'    => 'hazardous_and_universal_waste',
            'landfill'     => 'landfill_other',
            'other_materials' => 'other_materials',
        ];

        // safe extractor — returns float 0 when missing
        $getUnit = function($arr, $year, $month, $key) {
            if (empty($arr) || !isset($arr[$year]) || !isset($arr[$year][$month]) || !isset($arr[$year][$month][$key])) {
                return 0.0;
            }
            return (float) $arr[$year][$month][$key];
        };

        foreach ($categories as $key => $field) {
            $currVal = $getUnit($wasteMonthlyData, $currYear, $currMonth, "{$field}_unit_measure");
            $prevVal = $getUnit($wasteMonthlyData, $currYear - 1, $currMonth, "{$field}_unit_measure");
            $report[$key] = [
                'current'  => ['unit_measure' => $currVal],
        ...
        // Waste to Energy: checked streams with typical_destination 5 — wte_unit_measure
        $totalWteCurrent = $getUnit($wasteMonthlyData, $currYear, $currMonth, 'wte_unit_measure');
        $totalWtePrevious = $getUnit($wasteMonthlyData, $currYear - 1, $currMonth, 'wte_unit_measure');

        $totalRecyclingCurrent = $totalRecyclingWteCurrent - $totalWteCurrent;
        $totalRecyclingPrevious = $totalRecyclingWtePrevious - $totalWtePrevious;
        $report['recyclables'] = [
            'current'  => ['unit_measure' => $totalRecyclingCurrent],
            'previous' => ['unit_measure' => $totalRecyclingPrevious],
        ];

        // Total waste: YTD default — sum checked streams excluding typical_destination 7 only
        ...
        $previousDivertedPct = ($previousTotal > 0) ? ($totalRecyclingForDiversionPrevious / $previousTotal) * 100 : 0;

        $currentRecyclablesRate = ($currentTotal > 0) ? ($totalRecyclingCurrent / $currentTotal) * 100 : 0;
        $previousRecyclablesRate = ($previousTotal > 0) ? ($totalRecyclingPrevious / $previousTotal) * 100 : 0;

        $fmt = function($num, $decimal= 0) {
            return number_format((float)$num, $decimal, '.', ',');
        };

        /*
        |--------------------------------------------------------------------------
        | Final Waste Report Array
        |--------------------------------------------------------------------------
        */
        $wasteReportArray = [
            [
                ...
                'change'   => $fmt($percentChange($currentTotal, $previousTotal),2),
            ],
            [
                'metric'   => 'Organic / food waste (kg)',
                'current'  => $fmt($report['organic']['current']['unit_measure']),
                'previous' => $fmt($report['organic']['previous']['unit_measure']),
                'change'   => $fmt($percentChange(
                                $report['organic']['current']['unit_measure'],
                                $report['organic']['previous']['unit_measure']
                            ),2),
            ],
            [
                'metric'   => 'Recyclables (kg)',
                'current'  => $fmt($report['recyclables']['current']['unit_measure']),
                'previous' => $fmt($report['recyclables']['previous']['unit_measure']),
                'change'   => $fmt($percentChange(
                                $report['recyclables']['current']['unit_measure'],
                                $report['recyclables']['previous']['unit_measure']
                            ),2),
            ],
            [
                'metric'   => 'Waste To Energy',
                'current'  => $fmt($totalWteCurrent),
                'previous' => $fmt($totalWtePrevious),
                'change'   => $fmt($percentChange($totalWteCurrent, $totalWtePrevious), 2),
            ],
            [
                'metric'   => 'Hazardous waste (kg)',
                'current'  => $fmt($report['hazardous']['current']['unit_measure']),
                'previous' => $fmt($report['hazardous']['previous']['unit_measure']),
                'change'   => $fmt($percentChange(
                                $report['hazardous']['current']['unit_measure'],
                                $report['hazardous']['previous']['unit_measure']
                            ),2),
            ],
            [
                'metric'   => 'Waste diverted from landfill (%)',
                'current'  => $fmt($currentDivertedPct, 2),
                'previous' => $fmt($previousDivertedPct, 2),
                'change'   => $fmt($currentDivertedPct - $previousDivertedPct, 2),
            ],
            [
                'metric'   => 'Recyclables Rate',
                'current'  => $fmt($currentRecyclablesRate, 2),
                'previous' => $fmt($previousRecyclablesRate, 2),
                'change'   => $fmt($currentRecyclablesRate - $previousRecyclablesRate, 2),
            ],
        ];
        ...
        $wastePerGuest = [
            [
                ...
                'current'  => $fmt($currentRoomWaste, 2),
                'previous' => $fmt($previousRoomWaste, 2),
                'value'    => $fmt($percentChange($currentRoomWaste, $previousRoomWaste),2),
            ],
            [
                'metric'   => 'Organic waste (kg/Guest Night)',
                'current'  => $fmt($currentOrganic,2),
                'previous' => $fmt($previousOrganic,2),
                'value'    => $fmt($percentChange($currentOrganic, $previousOrganic),2),
            ],
            [
                'metric'   => 'Recyclables (kg/Guest Night)',
                'current'  => $fmt($currentRecyclables,2),
                'previous' => $fmt($previousRecyclables,2),
