...
$( function() {
	...
	$('.action-edit .editButtons #wpPreview').after('<input type="button" name="ImageUpload" id="ImageUpload" value="Upload Image" onclick="location.href=\'index.php/Special:Upload\'">');
	
	//$('#toggleLink').trigger('click');
	
	//for heading - Category:category name
	var categoryText = $('#content #firstHeading').text();
	if (categoryText.indexOf("Category:") >= 0)
	{
		var categoryTextChnged = categoryText.replace("Category:", "");
		$('#content #firstHeading').text(categoryTextChnged);
	}
...
$(window).load(function(){
   //custom js added
   $('#mw-content-text .image img').removeAttr('width');
   $('#mw-content-text .image img').removeAttr('height');
   $('#mw-content-text .image img').addClass('contentImage'); 
