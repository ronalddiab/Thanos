...
abstract class Skin extends ContextSource {
	...
	function getCategoryLinks() {
		...
		if ( !empty( $allCats['normal'] ) ) {
			...
			//get title of curret page - and from it fetch parent category name
			//Get the current page's Title
			$categoryName = strip_tags($t);
			
			

			//By default - below line - commented it - Dips
			//$s .= '<div id="mw-normal-catlinks" class="mw-normal-catlinks">' . $link . $colon . '<ul>' . $t . '</ul>' . '</div>';
			 
		}else{
			$categoryName = $this->getRelevantTitle()->getText();
		}
		
		$dbr = wfGetDB( DB_SLAVE );
			
		$res = $dbr->select(
			array('project_info as pi', 'project_categories as pc'),
			array('pi.project_name','pi.id'),
			array('pi.status = 1','pc.name = "'.$categoryName.'" ','pi.site_id = 0'),
			__METHOD__,
			array(),
			array( 'project_categories as pc' => array( 'INNER JOIN', array( 'pi.project_category_id=pc.id' ) ) )
		); 
		$arrURI = explode('/'.APPLICATION_NAME.'/', $_SERVER['REQUEST_URI']);
		if($res->result->num_rows > 0)
		{   
			$s.= '<div class="categoryContainer catlinks">
				<div class="projectTitleContainer"><h4 class="parentCat_h4">'.$categoryName.' actions:</h4></div>
				
				<div class="categoryListContainer">
					<ul class="categoryContainer_ul">';
			foreach( $res as $row ) { 
				$s .= '<li><a href="'.SERVER_NAME.$arrURI[0].'/'.APPLICATION_NAME.'/projects/actionlist/'.$row->id.'">'.$row->project_name.'</a></li>';	
			}
			$s.= '</ul></div></div>';
		}
	...
	function getCategories() {
		...
		//echo $this->getRelevantTitle()->getText();
		//exit;
		return "<div id='catlinks'>{$catlinks}</div>";
	}
